...
6. A Premium plate at eight counted washes: `pay`, `cap_reached`
...
- The count starts at the later of the month start and the plate's latest quota reset, so a reset restarts the cap mid-month. `Facts` in `core/internal/siteagent/store.go` reads it. The reset instant reaches the site through the change log in `canon/context/sync.md`.
...
- `GET /plates/{plate}` on central answers a plate's owner, subscription, this month's washes, washes since its latest reset, and the reset instant. A malformed plate answers 400 and a plate central holds no vehicle for answers 404.
