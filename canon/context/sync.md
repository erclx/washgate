...
- A batch is one transaction. An unknown site, company, or prepaid wash rolls the whole batch back and answers 422. A site can hold only prepaid wash ids central sent it, so an unknown one is a bug rather than a race.
- A newly stored `prepaid` wash marks its prepaid wash spent only where no spend is recorded yet, and then appends a `prepaid_spent` change. A second wash on an already spent prepaid wash is still stored and appends nothing. It is the offline double spend, and refusing it would lose a wash that happened.
...
- Every change carries a `kind`. A `plan` change sets a plate's plan, its company, or its quota reset. A `prepaid_granted` or `prepaid_spent` change names one `prepaid_wash_id`, carries no plan, and leaves the plate's plan alone. A change with no `kind` reads as `plan` at the site.
- Every write that changes a plate's plan, its company, or its quota reset, and every prepaid grant and first spend, appends a row to `entitlement_changes` in the same transaction. The writer first takes the single `entitlement_cursor` row with `FOR UPDATE`.
...
- A write to `vehicles` that changes none of the three appends nothing, because a site holds only Premium and fleet plates, their resets, and prepaid washes.
- The site inserts a granted prepaid wash with `ON CONFLICT DO NOTHING` and applies a spend as an update that only fills a null `spent_at`, so a late or repeated pull never gives a spent wash back. Central likewise fills `spent_wash_id` only where it is null.
- A prepaid wash is a row with an id at both ends rather than a count on the plate, because a count pulled while a local spend waits in the outbox would restore the spent wash.
...
- The same holds for a prepaid wash. A spend reaches another site only after the spending site pushes and the other site pulls, so two sites offline at once can each admit on one prepaid wash. Central keeps both washes and marks the first as the spend, and the second stays a `prepaid` wash against a spent prepaid wash, which is where an operator finds it.
...
- A wash carries `plan` of `premium`, `fleet`, or `prepaid`. Only a fleet wash names its company, and only a prepaid wash names its `prepaid_wash_id`. Central's request validator enforces both pairings before the store is touched.
...
- Central runs to `0005_prepaid_washes` and the site to `004_prepaid`. Site `004` rebuilds `washes` to widen its plan check, which `canon/context/development.md` describes.
...
A new write that alters a plate's plan, company, or quota reset goes through `lockEntitlementCursor` and `appendEntitlementChange` in `core/internal/central/store.go`, inside the transaction that makes the write. `ResetQuota` in `core/internal/central/operator_store.go` is the example for a change that carries a new field, and `GrantPrepaidWash` in `store.go` the example for a new `kind`. Add a site-side case in `applyChange` in `core/internal/siteagent/store.go` only if the change shape is new, since the site refuses a page holding a kind it does not know, and a column on `entitlement_changes` in a new central migration.
